 ...
 *
 * ─── MOVT_FNCR (lista de movimentações) ───────────────────────────────────
 * Query do BI usa RMS_AGT_FNCO (A) joined com FNDO_GRTR (B),
 * AGT_FNCO_FNDO_GRTR (C), AGT_FNCO (E), TIP_MTV_RJC_RMS (F), TIP_EST_RMS (G).
 * WHERE A.CD_TIP_NTZ_MVTC IN (1, 2) AND A.CD_FNDO_GRTR <> 1 (padrão do BI).
 *
 * Coluna TX_TIP_MTV_RJC_RMS (prefixo TX) conforme BI — verificar no schema real.
 * Coluna VL_LQDO_MVTC_RMS (corrigido — BI usa LQDO, não LODO).
 *
 * ─── DET_MVT_FNCR (detalhe por remessa) ──────────────────────────────────
 * RSM_MVTC_FNCR_RMS (A) INNER JOIN RMS_AGT_FNCO (B) INNER JOIN TIP_MVTC_FNCR (C).
...
public class MovimentacaoRepository {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // DETALHE DE UMA REMESSA — DET_MVT_FNCR (query exata do BI, custo 255.37)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // LISTA DE MOVIMENTAÇÕES — MOVT_FNCR (query fiel ao BI, custo 734.24)
    // ─────────────────────────────────────────────────────────────────────────
     ...
     *   [2]  CASE NM_TIP_NTZ_MVTC    tipoMovFinanceira  (texto resolvido)
    ...
    public List<Object[]> listar(int cdAgtFnco, int cdFundo, int cdTipEstRms,
        ...
        Query q = em.createNativeQuery(buildListSql())
                .setParameter(1, cdAgtFnco)
    ...
    private String buildListSql() {
        return "SELECT B.NM_FNDO_GRTR, "
            + "       A.NR_SEQL_RMS, "
            // tipo de movimentação: texto resolvido via CASE (BI — CD_TIP_NTZ_MVTC)
            ...
            + "       END, "
            + "       G.NM_TIP_EST_RMS, "
            // TX_TIP_MTV_RJC_RMS: prefixo TX conforme BI (verificar schema real)
            + "       F.TX_TIP_MTV_RJC_RMS, "
            + "       A.DT_PRCT_EVT, "
            + "       A.DT_ATL_MNTR, "
            + "       A.DT_MVTC_FNCR, "
            + "       A.QT_REG_RMS, "
            // VL_LQDO_MVTC_RMS: corrigido (BI usa LQDO, não LODO)
            + "       A.VL_LQDO_MVTC_RMS "
            + "FROM DB2GFG.RMS_AGT_FNCO A "
            + "INNER JOIN DB2GFG.FNDO_GRTR B ON A.CD_FNDO_GRTR = B.CD_FNDO_GRTR "
            + "INNER JOIN DB2GFG.AGT_FNCO_FNDO_GRTR C ON A.CD_FNDO_GRTR = C.CD_FNDO_GRTR "
     ...
     * CD_TIP_NTZ_MVTC IN (1,2) replicado do BI (tipos 'crédito do fundo' e 'crédito do agente').
     * CD_FNDO_GRTR <> 1 replicado do BI (exclui FGO Original).
     */
